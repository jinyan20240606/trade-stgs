...
class VolatilitySystemV5(IStrategy):
    """
    Volatility System V5 - Advanced Market Microstructure
    
    Key Improvements over V4:
    1. Volatility Clustering Detection: Avoid entries during extreme volatility spikes
    2. Volume Profile Analysis: Confirm trend strength with volume
    3. RSI Dynamic Filters: Prevent entries at extremes in range markets
    4. Adaptive Stoploss: Tighter stops in range, wider in trends
    
    Market States:
    - Strong Trend: High ADX, Volume > MA, Price > VWAP
    - Volatility Spike: High ATR deviation (danger zone)
    - Range: Low ADX, Normal Volume
    """
    can_short = True
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ATR Multiplier
        # Strong Trend: 0.8 (Aggressive)
        # Weak Trend: 1.0 (Normal)
        # Range: 1.8 (Conservative - increased from 1.5 in V4)
        # Volatility Spike: 2.0 (Very Conservative)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Entry logic with Volume and RSI filters
        """
        # === Long Entry ===
        long_condition = (
            # 1. Price breakout
